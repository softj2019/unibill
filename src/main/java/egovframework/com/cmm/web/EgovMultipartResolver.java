...
public class EgovMultipartResolver extends CommonsMultipartResolver {
	...
	for (Iterator<?> it = fileItems.iterator(); it.hasNext();) {
		...
		if (fileItem.getSize() > 0) {
			...
			LOGGER.debug("Found multipart file [{}] of size {} bytes", file.getName(), file.getSize());
			LOGGER.debug("with original filename [{}], stored {}", file.getOriginalFilename(), file.getStorageDescription());
		}
